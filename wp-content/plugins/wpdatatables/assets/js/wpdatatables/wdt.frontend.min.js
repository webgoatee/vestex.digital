...
fnOnFiltered()}
if(aoColumn.defaultValue){var defaultValue=jQuery.isArray(aoColumn.defaultValue)?aoColumn.defaultValue[0]:aoColumn.defaultValue;jQuery(input).val(defaultValue);oTable.api().column(columnIndex).search(defaultValue,bIsNumber,!1);if(!serverSide){jQuery(document).ready(function(){jQuery(input).trigger('keyup')});oTable.fnFilter(defaultValue,columnIndex)}}}
function wdtCreateNumberRangeInput(oTable,aoColumn,columnIndex,sColumnLabel,th,customSearchIndexes,serverSide){var tableId=oTable.attr('id');var fromDefaultValue='',toDefaultValue='',defaultValue=aoColumn.defaultValue;var tableDescription=jQuery.parseJSON(jQuery('#'+oTable.data('described-by')).val());var numberFormat=(typeof tableDescription.numberFormat!=='undefined')?parseInt(tableDescription.numberFormat):1;var replaceFormat=numberFormat===1?/\./g : /,/g;if(defaultValue!==''){fromDefaultValue=defaultValue[0];toDefaultValue=defaultValue[1]}
...
if(iMin!==''){iMin=iMin*1}
if(iMax!==''){iMax=iMax*1}
iValue=iValue*1;return(iMin===""&&iMax==="")||(iMin===""&&iValue<=iMax)||(iMin<=iValue&&""===iMax)||(iMin<=iValue&&iValue<=iMax)});jQuery('#'+sFromId+', #'+sToId,th).keyup(function(){numberRangeSearch()});if(fromDefaultValue){jQuery(from).val(fromDefaultValue);oTable.api().column(columnIndex).search(fromDefaultValue);if(!serverSide){jQuery(document).ready(function(){jQuery(from).keyup()})}}
if(toDefaultValue){jQuery(to).val(toDefaultValue);oTable.api().column(columnIndex).search(toDefaultValue);if(!serverSide){jQuery(document).ready(function(){jQuery(to).keyup()})}}
...
fnOnFiltered()});if(fromDefaultValue){jQuery(from).val(fromDefaultValue);oTable.api().column(columnIndex).search(fromDefaultValue);if(!serverSide){jQuery(document).ready(function(){jQuery(from).trigger('blur')})}}
if(toDefaultValue){jQuery(to).val(toDefaultValue);oTable.api().column(columnIndex).search(toDefaultValue);if(!serverSide){jQuery(document).ready(function(){jQuery(to).trigger('blur')})}}}
...
fnOnFiltered()});if(fromDefaultValue){jQuery(from).val(fromDefaultValue);oTable.api().column(columnIndex).search(fromDefaultValue);if(!serverSide){jQuery(document).ready(function(){jQuery(to).trigger('blur')})}}
if(toDefaultValue){jQuery(to).val(toDefaultValue);oTable.api().column(columnIndex).search(toDefaultValue);if(!serverSide){jQuery(document).ready(function(){jQuery(to).trigger('blur')})}}}
...
r+='<div class="wdt_checkbox_option checkbox">'+'<label>'+'<input type="checkbox" class="wdt-checkbox-filter wdt-filter-control" value="'+encodeURI(value)+'" '+checked+'>'+'<i class="input-helper"></i>'+label+'</label>'+'</div>'}
...
return data},preserveSelected:!0,emptyRequest:!0,preserveSelectedPosition:'before',locale:{emptyTitle:wpdatatables_frontend_strings.nothingSelected,statusSearching:wpdatatables_frontend_strings.sLoadingRecords}})}
...
if(tableDescription.serverSide){dataTableOptions.ajax.data=function(data){data.sumColumns=sumColumns;data.avgColumns=avgColumns;data.minColumns=minColumns;data.maxColumns=maxColumns;data.currentUserId=$('#wdt-user-id-placeholder').val();data.currentUserLogin=$('#wdt-user-login-placeholder').val();data.currentPostIdPlaceholder=$('#wdt-post-id-placeholder').val();data.wpdbPlaceholder=$('#wdt-wpdb-placeholder').val();data.wdtNonce=$('#wdtNonceFrontendEdit').val()}}
